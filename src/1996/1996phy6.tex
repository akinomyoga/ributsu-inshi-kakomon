...
\SubQuestion
  ...
  \SubSubQuestion
    ...
    $\Vec{v} = -\Grad{\uchi}$ となる速度ポテンシャル $\uchi(\Vec{r},t)$
    が存在してラプラスの方程式
%
    \begin{equation}
      \Laplacian{\uchi (\Vec{r},t)} = 0 \eqname{Q1}
    \end{equation}
%
    を満たすことを示せ。
...
%
    \begin{equation}
      \uchi(\Vec{r},t) = \sum_{\ell m} a_{\ell m}(t)f_\ell(r)Y_{\rm\ell m}(\theta,\uphi) \eqname{Q2}
    \end{equation}
%
    と書ける。このとき、動径関数は
%
...
\SubQuestion
  液滴の重心から$(\theta,\uphi)$ 方向を見たとき、表面までの距離が
%
  \begin{equation}
    R(\theta,\uphi,t) = R_0 + q(t)Y_{\rm 20}(\theta,\uphi)
    \eqname{Q4}
  \end{equation}
%
...
%
    \begin{equation}
      \Partial{}{t} R(\theta,\uphi,t)%
      = -\Partial{}{r} \uchi (r,\theta,\uphi,t) \bigm|_{r=R}%
      \eqname{Q5}
    \end{equation}
%
    の関係が成り立つとして、速度ポテンシャル $\uchi(\Vec{r},t)$ を 
    $\dot{q}(t) \left( \equiv \tDeriver{q}{t} \right) $ を用いて
    表せ。ただし、変形は微小なので、右辺で $r=R$ を $r=R_0$ と
    近似して良いものとする。

  \SubSubQuestion
    液滴内部の速度分布 $(v_x,v_y,v_z)$ を、$Y_{20}(\theta,\uphi)%
...
%
    ...
    対するポテンシャル$\Vec{v}=-\Grad{\uchi}$が存在する。
    これの$\Div{}$を計算すると、以下の通りラプラスの方程式\eqhref{Q1}
    が示される。
%
    \[ \Div{\Vec{v}}=-\Div{\Grad{\uchi}} =-\Laplacian{\uchi} =0 \]
%
  \SubSubAnswer
...
%
    である。よって$\uchi$は式\eqhref{Q2}より次の通り。
%
    \[ \uchi = a_{20}(t)r^2Y_{20}(\theta, \varphi)%
