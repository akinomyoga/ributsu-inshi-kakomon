...
図\ref{2004phy4-3}のように配置して実験を行った。なお、$D$は波高分析装置 (PHA) に接続されており、エネルギー$E$をもつガンマ線だけをカウントすることができる。
...
図\ref{2004phy4-4}(a), (b)にはこのうち、２つを模式的に示し、
また、図\ref{2004phy4-5}のA, B, Cの曲線は、ゲルマニウムに対する、それぞれからの$\mu$への寄与を示す。
図\ref{2004phy4-4}(a)および(b)の過程はそえぞれどう呼ばれているか、またそれぞれが図\ref{2004phy4-5}のA, B, Cのどれに対応するかを答えよ。
さらに、残りの過程の名称を答え、その特徴を示す模式図を図\ref{2004phy4-4}にならって描け。
...
\begin{enumerate}
\item
図\ref{2004phy4-6}(a)のように、ある励起した原子核$A^{*}$は、その静止系で、1.4MeVのエネルギーのガンマ線を放出して原子核Aに崩壊する。
実験室で原子核反応により励起した原子核$A^{*}$を生成したところ、実験室系での速さが光速$c$の80\%であった。
図\ref{2004phy4-6}(b)に示すように、実験室系で、崩壊前の$A^{*}$の速度ベクトルの方向に対するガンマ線の放出角度を$\theta_L$とする。$\theta_L$=60°でガンマ線を観測すると、観測されるガンマ線のエネルギーがいくらになるか。有効数字２桁で答えよ。
