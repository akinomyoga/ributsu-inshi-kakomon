...
活量$a_{k}$は以下の式で定義される:
\begin{eqnarray}
\mu_{k} (T,p) = \mu^{\ 0}_{k} (T) + k_{B}T\ln a_{k}. \label{eq.1}
\end{eqnarray}
また,\ 物質$k$と物質$k'$の間での\underline{二状態転移における}平衡定数$K(T)$は次式で定義される:
\begin{eqnarray}
K(T) = \frac{a_{k'}}{a_{k}}. \label{eq.2}
\end{eqnarray}
...
\begin{eqnarray}
\Delta G &=& N \left(\mu^{\ 0}_{k'} (T) - \mu^{\ 0}_{k} (T) \right), \label{eq.5} \\
&=&-N k_{B} T \ln K(T) ,\nonumber\\
...
\end{eqnarray}
$\Delta G= \Delta H- T \Delta S$を,\ 上の式と式(\ref{eq.5})の二行目を用いて変形すると,\
\begin{eqnarray}
