...
\begin{subquestions}
\SubQuestion
  式\ref{Q01}はGaussの法則を表す。
  \begin{subsubquestions}
  \SubSubQuestion
  閉じた中空の導体があると、導体の外部にある電荷が作る電場は、導体に囲
  まれた中空の導体には侵入できない。このことを、式\ref{Q01}を用いて証明せよ。
...
\SubAnswer
  ...
  \SubSubAnswer
    \parbox[t]{100mm}{
   中空の空間には電荷が存在しない($\rho=0$)ので、\ref{Q01}より$\Div \vec{E}=0
   $。\\
...
%   \vspace{1mm}
 ...
 \SubAnswer
  ...
  \SubSubAnswer
    ...
    これで、\ref{Q04}が導かれた
  \SubSubAnswer
    局所的な$\vec{j}$が一瞬生じると、\ref{Q04}によって近傍に$\vec{B}$
    が生じ、$\vec{B}$の変化があるので\ref{Q03}によって近傍に$\vec{E}$が生じ、
    $\vec{E}$の変化があるので、再び\ref{Q04}によって近傍に$\vec{B}$が生じる\\
    $\vec{j}$が消滅した後\ref{Q02}も、引き続き\ref{Q03},\ref{Q04}
    によって$\vec{E}$,$\vec{B}$の
  ...
  \SubSubAnswer
    ...
    \end{eqnarray}
    が得られる。\\
    \ref{c5},\ref{c6}は波動方程式であるから、電磁場の存在が示された。
    位相速度は
    \[
  ...
  \SubSubAnswer
    $\Div \Grad=0$であるから、\ref{Q02}より
    \begin{eqnarray}
      \vec{B}=\Rot \vec{A}
      \label{31}
    \end{eqnarray}
    なるベクトル場$\vec{A}$が存在することが分かる。\\
    \ref{31}を\ref{Q03}に代入すると
